...
fn build_random_leaf(taffy: &mut Taffy) -> NodeKey {
    taffy.new_with_children(Style::DEFAULT, &[]).unwrap()
}

/// A tree with many children that have shallow depth
fn build_taffy_flat_hierarchy(total_node_count: u32, use_with_capacity: bool) -> (Taffy, NodeKey) {
    ...
    while node_count < total_node_count {
        ...
        let sub_children: Vec<NodeKey> = (0..sub_children_count).map(|_| build_random_leaf(&mut taffy)).collect();
