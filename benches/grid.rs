...
fn build_random_leaf(taffy: &mut Taffy, _rng: &mut ChaCha8Rng) -> NodeKey {
...
fn build_grid_flat_hierarchy(col_count: usize, row_count: usize) -> (Taffy, NodeKey) {
...
pub fn build_deep_grid_tree(
    ...
    levels: usize,
    track_count: usize,
    create_leaf_node: &mut impl FnMut(&mut Taffy) -> NodeKey,
    create_container_node: &mut impl FnMut(&mut Taffy, Vec<NodeKey>) -> NodeKey,
) -> Vec<NodeKey> {
...
fn build_taffy_deep_grid_hierarchy(levels: usize, track_count: usize) -> (Taffy, NodeKey) {
    let mut rng = ChaCha8Rng::seed_from_u64(12345);
    let mut build_leaf_node = |taffy: &mut Taffy| build_random_leaf(taffy, &mut rng);
    let mut rng = ChaCha8Rng::seed_from_u64(12345);
    let mut build_flex_node = |taffy: &mut Taffy, children: Vec<NodeKey>| {
