...
pub(super) fn place_grid_items<'a, ChildIter, K>(
    ...
    ChildIter: Iterator<Item = (usize, K, &'a Style)>,
    K: NodeKey,
{
    ...
    let map_child_style_to_origin_zero_placement = {
        ...
        move |(index, node, style): (usize, K, &'a Style)| -> (_, _, _, &'a Style) {
...
fn record_grid_placement<K: NodeKey>(
    cell_occupancy_matrix: &mut CellOccupancyMatrix,
    items: &mut Vec<GridItem<K>>,
    node:K,
    index: usize,
...
mod tests {
    ...
    mod test_placement_algorithm {
        ...
        fn placement_test_runner(
            ...
            children: Vec<(usize, NodeKey, Style, ExpectedPlacement)>,
