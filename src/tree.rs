...
pub trait LayoutTree<K = DefaultKey> where K: 'static + Copy + Clone + Default + Eq + PartialEq {
    /// Type representing an iterator of the children of a node
    type ChildIter<'a>: Iterator<Item = &'a K>
    where
        Self: 'a;

    /// Get the list of children IDs for the given node
    fn children(&self, node: K) -> Self::ChildIter<'_>;

    /// Get the number of children for the given node
    fn child_count(&self, node: K) -> usize;

    /// Returns true if the node has no children
    fn is_childless(&self, node: K) -> bool;

    /// Get a specific child of a node, where the index represents the nth child
    fn child(&self, node: K, index: usize) -> K;
    ...
    fn style(&self, node: K) -> &Style;

    /// Get the node's output "Final Layout"
    fn layout(&self, node: K) -> &Layout;

    /// Modify the node's output layout
    fn layout_mut(&mut self, node: K) -> &mut Layout;
    ...
    fn mark_dirty(&mut self, node: K) -> TaffyResult<()>;
    ...
    fn needs_measure(&self, node: K) -> bool;

    /// Get a cache entry for this Node by index
    fn cache_mut(&mut self, node: K, index: usize) -> &mut Option<Cache>;
}
